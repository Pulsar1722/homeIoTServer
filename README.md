...
最初の家族が帰宅したときに実行する家電操作は、ブラウザから設定できます。

- 設定画面: `http://<サーバーのホスト名>:<ポート>/settings.html`
- 保存すると `automations.json` が作成され、次回の帰宅時から反映されます。
- `automations.json` がない場合は、リポジトリの `automations.default.json` を初期設定として実行します。
- 両方のファイルがない場合、帰宅時の追加家電操作は実行しません。

設定画面では、実行順に機器コマンドと待機を追加できます。各項目には有効／無効の設定があり、無効な項目は削除せずに保存したまま実行対象から外せます。たとえば、夏以外はエアコンの項目を無効にしておき、夏になったら再度有効にできます。

### エアコン（赤外線リモコン）の設定

エアコンは `turnOn` ではなく、`setAll` コマンドを使用します。
...
`parameter` は `設定温度,運転モード,風量,電源` の順です。

- 運転モード: `1`=自動、`2`=冷房、`3`=除湿、`4`=送風、`5`=暖房
- 風量: `1`=自動、`2`=弱、`3`=中、`4`=強
- 電源: `on` または `off`

設定画面と設定APIにはアプリ内認証がありません。インターネットに公開する場合は、リバースプロキシなどで `/settings.html` と `/api/` に認証を設定してください。
